<?php
	namespace sda17dev;
	class SDA_SearchQuery
	{
		/**
		 * The SDA object this class will use to access the API.
		 *
		 * @var SDA
		 */
		protected $_SDA = null;
		 ...
		 *
		 * @param SDA  $SDA    The SDA object to use.
		 * @param array $options An initial set of search options.
		 */
		public function __construct($SDA, $options = array())
		{
			if (!($SDA instanceof SDA)) {
				throw new Exception('Please provide an object of type \TFN\SDA');
			}
			$this->_SDA = $SDA;
		 ...
		 *
		 * @return SDA_SearchResults
		 */
		public function execute()
		{
			if (!class_exists('\TFN\SDA_SearchResults')) {
				require dirname(__FILE__).'/searchresults.php';
			}
			return new SDA_SearchResults(
				$this->_SDA,
				$this->_options,
				$this->_SDA->callAPI('items', $this->_options));
		}
		/**
